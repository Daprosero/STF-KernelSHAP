...
from Model_utils.stf_kernelshap import stf_kernelshap_all_xtest
...
def get_classification_output(y):
...
def get_sample(X, sample_idx):
    return X[sample_idx:sample_idx + 1].astype(np.float32)
...
def make_logits_model(
    ...
    Caso 2: ShallowConvNet
        Dense(..., activation=None, name="output")
        Activation("softmax", name="out_activation")

    En ambos casos devuelve un modelo con la misma entrada y salida logits.
    """
    ...
    if isinstance(output_layer, tf.keras.layers.Dense):
        ...
        logits_layer = logits_model.get_layer(logits_layer_name)
        logits_layer.set_weights(output_layer.get_weights())
...
def predict_scores(
    ...
    """
    Devuelve la salida usada como score para XAI.

    Esta función se mantiene como wrapper por compatibilidad.

    Si use_logits=False:
        usa el modelo original.

    Si use_logits=True:
        construye el modelo de logits una vez para esta llamada.

    Para métodos pesados como KernelSHAP, STF-KernelSHAP y Occlusion,
    es preferible usar:
        get_score_model(...)
